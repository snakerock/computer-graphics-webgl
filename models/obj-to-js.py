...
def convert_ascii(infile, morphfiles, colorfiles, outfile):
    ...
    "vertexNormals": ",".join(generate_normal(n) for n in generate_vertex_normals(faces, normals, len(vertices))),
...
def generate_vertex_normals(faces, normals, vlen):
    vertexNormals = []
    ...
    for f in faces:
        ...
        for i in xrange(nVertices):
            for j in xrange(3):
                vertexNormals[f['vertex'][i] - 1][j] += normals[f['normal'][i] - 1][j]
            normalize(vertexNormals[f['vertex'][i] - 1])
